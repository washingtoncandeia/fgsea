...
res.gbs <- results(dds, contrast = c('condition', 'gbs', 'control'), filterFun=ihw, alpha = 0.05)
res.gbs.rec <- results(dds, contrast = c('condition', 'gbs_rec', 'control'), filterFun=ihw, alpha = 0.05)

summary(res)

write.csv(as.data.frame(res), file = './GSEA/fgsea_ZGBS_zika_vs_control_SYMBOL.csv')
write.csv(as.data.frame(res.gbs), file = './GSEA/fgsea_ZGBS_gbs_vs_control_SYMBOL.csv')
write.csv(as.data.frame(res.gbs.rec), file = './GSEA/fgsea_ZGBS_gbs_rec_vs_control_SYMBOL.csv')
